...
선수들의 이름이 1등부터 현재 등수 순서대로 담긴 문자열 배열 `players`와 해설진이 부른 이름을 담은 문자열 배열 `callings`가
...
function solution(players, callings) {
    // 선수들의 현재 순위를 저장하는 해시 테이블을 생성합니다.
    let indexMap = new Map(players.map((player, index) => [player, index]));

    for(let i = 0 ; i < callings.length; i++){
        //get index of callings[i] from hash
        let currentPlayerIndex = indexMap.get(callings[i]); //3
        let previousPlayer = players[currentPlayerIndex - 1];

        //swap players
        players[currentPlayerIndex - 1] = callings[i];
        players[currentPlayerIndex] = previousPlayer;

        //store the new values into hash
        indexMap.set(callings[i], currentPlayerIndex - 1);
        indexMap.set(players[currentPlayerIndex], currentPlayerIndex);
     }

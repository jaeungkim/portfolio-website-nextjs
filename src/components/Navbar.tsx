...
export default function Navbar() {
  ...
  const themeButtonClass = useMemo(() => {
    return `group rounded-full bg-white/90 px-3 py-2 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur transition dark:bg-zinc-800/90 dark:ring-white/10 ${theme === "dark"
  ...
  const icon = useMemo(() => {
    return theme === "dark" ? (
      ...
      <p className="w-5 h-5 text-blue-500" />
    );
  }, [theme]);
